...
| | 地址 | 用途 |
|---|---|---|
| WebSocket | `ws://127.0.0.1:5000/` | 扫描：取设备列表、扫描、设置项 |
| HTTP | `http://127.0.0.1:18080` | 取图片、转发上传、查状态 |
...
- 失败时只回一条 `code:-1`，`msg` 说明原因（见 7.1 的对照表）。
...
**扫描本身目前只能走 WebSocket**（`/api/scan` 是给调试用的，格式和上面这套不一致，不要在客户端用）。
