...
只实时读分辨率是有意的：DLL 读其它能力时会把数据源临时 enable 到 state 5，
个别设备会因此空走一次纸或者亮灯，不适合放在随手就调的 GET 上。
...
⚠ 读取时 DLL 会把数据源临时 enable 到 state 5 再 disable 回来，**个别设备会空走一次纸或亮灯**。
别在轮询里调它，连上设备后查一次、前端缓存起来即可。
...
| `{"handle":"getScannerOptions","scanner":"设备名"}` | 先连接该设备，再现读能力，按前端的选项模型返回，见 8.3 |
...
- 前端在切换扫描仪时就会自动发这条，所以**选中扫描仪就会打开设备**；设备打不开回 `code:-1`
- 要读十来项能力，DLL 每读一项都会把数据源临时 enable 再 disable，真实设备上可能会有几次亮灯/动作
