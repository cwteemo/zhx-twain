...
设置项由服务端配置决定，会随着适配的扫描仪继续增加。前端按 3.2 的规则通用渲染即可，服务端加项时前端不用改。

表里的 `control` 是**常见情况**，实际以返回值为准：数值类的形态跟着扫描仪走——
报区间给 `slider`，报固定几档给 `select`，只报一个值给 `number`。
比如 `dpi` 多数机器是 `select`，`threshold` 多数是 `slider`，但换一台机器可能不同。
